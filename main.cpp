...
#include <QImage>
#include <cstdint> //quitarlo
#include <cstdlib> //quitarlo
#include "infomain.h"
using namespace std;
...
int main(){
    ...
    //crear el main con lo dado por los profesores
    QString archivoEntrada = "C:/Users/Solanyi/Desktop/desafio1/Code/I_M.bmp"; //imagen distorsionada (GAUSSEANA)
    QString archivoSalida = "C:/Users/Solanyi/Desktop/imagen1.bmp";  //ruta para guardar imagen/IMAGEN ORIGINAL
    QString imagenmascara="C:/Users/Solanyi/Desktop/desafio1/Code/M.bmp";  //imagen de la mascara
    const char* archivotxt="C:/Users/Solanyi/Desktop/desafio1/Caso 2/M6.txt";


    //loadpixels imagen distorsionada (se trabaja con ella al final)
    int altoimagen = 0;
    int anchoimagen = 0;
    unsigned char *basedatosimagen = loadPixels(archivoEntrada, anchoimagen, altoimagen); //guarda base de datos imagendistor




    //loadpixels imagen de la mascara
    int altomascara=0;
    int anchomascara=0;
    unsigned char *basedatosmascara = loadPixels(imagenmascara, anchomascara, altomascara);//guarda base de datos de la imagen de la mascara
    int semilla  = 0;
    int numpixeles = 0;
    int tamano=(altomascara*anchomascara*3); //dimensiones de la mascara

    unsigned int *infotxt=loadSeedMasking(archivotxt, semilla, numpixeles); //info del txt
    unsigned char *quitarmascara=quitar_mascara(basedatosmascara,infotxt,tamano);

    //aplicar XOR en el pedazo de imagen resultante de quitar la mascara

    unsigned char* XOR= opxor(quitarmascara,infotxt , tamano );
    ...
    /*for (int i = 0; i < numpixeles * 3; i += 3) {
        cout << "Pixel " << i / 3 << ": ("
             << quitarmascara[i] << ","
             << quitarmascara[i + 1] << ", "
             << quitarmascara[i + 2] << ")" << endl;
    }*/
    /*si imprime, en algunas partes imprime alfanumericos y en otras numeros, a veces en los numeros hay espacios
        esto sucede cuando la funciones unsigned char, pero cuando es int lanza numeros por debajo de 250
        pero despues de cierto pixel todo se pone cero (0, 0, 0 )*/

    //FUNCIONA. LA FUNCION IMAGEN ROTADA FUNCIONA, SOLO HAY QUE CAMBIAR ALGUNAS COSAS
    //funcion que rota los pixeles, numrotar cantidad de numeros que quiere rotar, derecha:si rota a la derecha o no 1 si 0 no
    //aqui no seria procesar toda la imagen, sino lo que quedo de la resta de la mascara
//    unsigned int cantpixeles=altomascara*anchomascara; //alto mascara y ancho mascara
//    unsigned int numrotar=2;
//    unsigned int derecha=1;
//    unsigned char* imagenRotada = Rotaciones(quitarmascara, numpixeles, numrotar, derecha); //base de datos de la resta


    //despues de aplicarle alguna transformacion, se compara el resultado con el txt inmmediatamente anterior
    //bool estransformacion=comparar(imagenRotada,basedatostxtanterior,numpixeles)
    ...
    //comparar el puntero de arreglo que sale del proceso anterior con otra funcion y ver si si son iguales.
    //si son iguales, entonces ahi si aplicar el cambio a toda la imagen

    //bool exportI = exportImage(&imagenrotada, anchoimagen ,altoimagen, archivoSalida);
    ...
    /*for (int i = 0; i < width * height * 3; i += 3) {
        cout<< basedatosimagen[i] = [i];     // Canal rojo
        cout<<basedatosimagen[i + 1] = i; // Canal verde
        cout<<basedatosimagen[i + 2] = i; // Canal azul
    }
    */
    //este proceso de la imagen es despues de haber aplicado los procesos necesarios
    // Exporta la imagen modificada a un nuevo archivo BMP

    //bool exportI = exportImage(basedatosimagen, alnchoimagen ,altoimagen, archivoSalida);

    // Muestra si la exportación fue exitosa (true o false)
    //cout << exportI << endl;*/

    //funcion que retorna puntero de arreglos de la info del txt
    ...
    delete[] basedatosimagen;
    basedatosimagen = nullptr;
    delete[]basedatosmascara;
    basedatosmascara=nullptr;
    delete[]quitarmascara;
    quitarmascara=nullptr;
